...
export function PuzzleSolvingPage() {
  ...
  const loadNewPuzzle = useCallback(async () => {
    try {
      ...
      toast.error(error.message || "Failed to load puzzle");
      setLoading(false);
    }
  }, [difficulty]);
  ...
  const legalMoves = useMemo(() => {
    ...
    return game.moves({ square: square as any, verbose: true }).map(m => ({
  ...
  return (
    <div className="min-h-screen bg-[#0A0F2C] text-white flex flex-col font-['Inter']">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className={`absolute -top-1/4 -right-1/4 w-[60%] h-[60%] bg-gradient-to-br ${config.color} opacity-5 blur-[150px] rounded-full`} />
      ...
      <header className="relative z-10 w-full px-6 py-4 bg-[#11193F]/40 backdrop-blur-xl border-b border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-3">
             ...
             <div>
                <h1 className="text-lg font-bold leading-none mb-1">{config.name} Puzzle</h1>
                <p className="text-xs text-[#C9CAD9] opacity-60 uppercase tracking-wider font-semibold">Tactical Training</p>
             </div>
          </div>
        </div>
    ...
    Tactical precision achieved ♞
              ...
              {isWrong && (
                <motion.div 
                  ...
                  className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none"
                >
                  <div className="bg-red-500/20 backdrop-blur-md border border-red-500/50 p-8 rounded-3xl shadow-2xl flex flex-col items-center gap-4">
                    <AlertCircle className="w-16 h-16 text-red-400" />
                    <span className="text-2xl font-bold text-white">Try Again</span>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            {/* Board Glow Effect */}
            <div className={`absolute -inset-4 bg-gradient-to-br ${config.color} opacity-10 blur-2xl rounded-2xl`} />
            
            <div className={`relative w-full h-full bg-[#11193F]/60 backdrop-blur-sm rounded-xl border border-white/10 p-2 shadow-2xl transition-all ${isWrong ? 'ring-4 ring-red-500/50' : isSolved ? 'ring-4 ring-emerald-500/50' : ''}`}>
              {loading ? (
        ...
        <div className="w-full lg:w-[400px] flex flex-col gap-6">
          ...
          <section className="bg-[#11193F]/40 backdrop-blur-xl rounded-[2rem] border border-white/5 p-8">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h2 className="text-xl font-bold">Puzzle #{puzzleId?.slice(-4) || '----'}</h2>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/5">
                ...
                <span className={`font-bold ${isSolved ? 'text-emerald-400' : 'text-blue-400'}`}>
