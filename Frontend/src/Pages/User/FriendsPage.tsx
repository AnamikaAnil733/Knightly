          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-6">
            <div>
              ...
              <p className="text-[#9ca3af] mt-1">
                Manage your friends and challenge them to a match.
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-4 mb-6 border-b border-white/5">
            ...
            >
              ...
              {pendingRequests.length > 0 && (
                <span className="bg-[#EF476F] text-white text-[10px] px-1.5 py-0.5 rounded-full">
                ...
                <div className="bg-[#11193F]/50 border border-dashed border-[#FFD166]/20 rounded-2xl p-6 mb-8">
                  ...
                  <div className="space-y-3">
                    {searchResults.map((user) => (
                      <div
                        key={user.id}
                        className="flex items-center justify-between bg-[#0A0F2C] p-4 rounded-xl border border-white/5"
                      >
                        ...
                        <button
                          onClick={() =>
                            handleAddFriend(user.id, user.displayname)
                          }
                  ...
                  <div className="bg-[#EF476F]/10 border border-[#EF476F]/30 rounded-2xl p-6 mb-8 animate-in fade-in slide-in-from-top-4 duration-500">
                    ...
                    <div className="space-y-3">
                      {pendingRequests.map((req) => (
                        <div
                          key={req.id}
                          className="flex items-center justify-between bg-[#11193F] p-4 rounded-xl border border-white/5"
                        >
                          <div className="flex items-center gap-3">
                            <img
                              src={
                                req.avatarUrl ||
                                `https://api.dicebear.com/7.x/bottts/svg?seed=${req.displayname}`
                              }
                              className="w-10 h-10 rounded-full"
                              alt=""
                            />
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              ...
                              className="px-3 py-1.5 bg-white/5 text-[#9ca3af] rounded-lg font-bold text-xs hover:bg-white/10 transition-all"
                            >
                              Reject
                            </button>
                          </div>
            ...
            <div className="space-y-4">
              {pendingRequests.length === 0 ? (
                ...
                pendingRequests.map((req) => (
                  <div
                    key={req.id}
                    className="flex items-center justify-between bg-[#11193F] p-4 rounded-2xl border border-white/5 hover:border-[#FFD166]/30 transition-all"
                  >
                    <div className="flex items-center gap-4">
                      <img
                        ...
                        className="w-12 h-12 rounded-full border-2 border-[#FFD166]/20"
                        alt=""
                      />
                      <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => handleRejectRequest(req.id)}
                        className="px-4 py-2 bg-white/5 text-[#9ca3af] rounded-xl font-bold text-sm hover:bg-white/10 transition-all"
                      >
                        Reject
                      </button>
                    </div>
