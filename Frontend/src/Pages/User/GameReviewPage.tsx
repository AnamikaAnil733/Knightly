...
export function GameReviewPage() {
  ...
  useEffect(() => {
    if (fens.length > 0 && currentMoveIndex >= 0 && currentMoveIndex < fens.length) {
  ...
  const currentReview = currentMoveIndex > 0 ? reviewData[currentMoveIndex - 1] : null;
  ...
  const getClassificationColor = (classification: string) => {
      switch(classification) {
          ...
          case 'BEST': return 'text-green-300 font-bold';
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden p-4 gap-4">
        ...
        <div className="w-full lg:w-[400px] flex flex-col bg-[#11193F]/50 rounded-xl border border-white/10 overflow-hidden">
           ...
           <div className="p-4 border-b border-white/10 bg-black/20 flex flex-col items-center justify-center min-h-[100px]">
              {currentMoveIndex === 0 ? (
                  <h2 className="text-xl font-bold text-gray-300">Starting Position</h2>
              ) : currentReview ? (
                  <div className="text-center">
                     <div className={`text-3xl font-bold mb-1 ${getClassificationColor(currentReview.classification)}`}>
                         {currentReview.classification} {getClassificationSymbol(currentReview.classification)}
                     </div>
                     <p className="text-sm text-gray-400">
                         Engine Eval: {currentReview.evaluation?.mate ? `Mate in ${currentReview.evaluation.mate}` : `${(currentReview.evaluation?.score / 100).toFixed(2)}`}
                     </p>
                  </div>
              ) : (
                  <h2 className="text-xl font-bold text-gray-300">Move {currentMoveIndex}</h2>
              )}
           </div>
           ...
           <div className="flex-1 overflow-y-auto p-2 space-y-1">
             {reviewData.map((data, idx) => {
                ...
                return (
                   ...
                   >
                      <span className="text-gray-400 w-8">{isWhite ? moveNum + "." : ""}</span>
                      <span className="flex-1 font-mono">{moveText}</span>
                      <span className={`font-bold ${getClassificationColor(data.classification)}`}>
           ...
           <div className="p-4 border-t border-white/10 bg-black/20 flex justify-center gap-2">
              ...
              <button 
                onClick={() => setCurrentMoveIndex(c => Math.min(fens.length - 1, c + 1))}
