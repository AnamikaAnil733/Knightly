...
import { Lock, Trophy, Award, Target, Flame, Medal, Star, Zap } from 'lucide-react';
...
const AchievementCard: React.FC<AchievementCardProps> = ({
  ...
  const renderIcon = () => {
    const props = { 
      size: 40, 
      className: isEarned ? "text-yellow-400" : "text-gray-500 transition-all duration-700" 
    };
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col items-center text-center gap-4 flex-grow">
          <div className={`p-5 rounded-3xl relative transition-all duration-700 ${
            isEarned 
          ...
          <div className="space-y-2">
            <h3 className={`text-lg font-bold tracking-tight ${isEarned ? "text-yellow-500" : "text-gray-400"}`}>
              {title}
            </h3>
            <p className={`text-sm leading-relaxed ${isEarned ? "text-gray-400" : "text-gray-600"}`}>
              {description}
            </p>
          </div>
        </div>
        ...
        {isEarned && earnedAt && (
          <div className="mt-6 pt-4 border-t border-gray-800/50 w-full text-center">
            <span className="text-[10px] uppercase tracking-[0.2em] text-yellow-500/50 font-bold">
              Unlocked {new Date(earnedAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
            </span>
          </div>
        )}
      </motion.div>
