    ...
    >
          ...
          <div className="flex items-center gap-2">
            <p
              className={`font-bold transition-colors ${
                isPremium && !user?.premium
                  ? "text-gray-500"
                  : "text-white group-hover:text-[#FFD166]"
              }`}
            >
              {title}
            </p>
            {isPremium && (
              <Lock
                className={`w-3.5 h-3.5 ${user?.premium ? "text-green-500" : "text-amber-500 fill-amber-500/10"}`}
              />
            )}
          </div>
          <span
      ...
      <ChevronRight
        className={`w-5 h-5 transition-colors flex-shrink-0 ${
          isPremium && !user?.premium
            ? "text-white/5"
            : "text-white/20 group-hover:text-[#FFD166]"
        }`}
      />
    </button>
  );
