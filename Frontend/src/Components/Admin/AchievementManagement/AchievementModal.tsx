      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1.5">Criteria Type</label>
              <select
              ...
              >
                {CRITERIA_OPTIONS.map((opt) => (
                  <option key={opt.value} value={opt.value} className="bg-[#0A0F2C]">
                    {opt.label}
                  </option>
                ))}
              </select>
            ...
            <div>
              ...
              />
              {errors.criteriaValue && <p className="mt-1 text-xs text-red-400">{errors.criteriaValue}</p>}
            </div>
          </div>

          {/* Preview */}
          <div className="flex items-center gap-4 p-4 rounded-xl border border-[#FFD166]/20 bg-[#FFD166]/5">
            <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-gradient-to-br from-[#6B2EFF]/40 to-[#3A6FF7]/40
                            flex items-center justify-center border border-[#FFD166]/30">
              <SelectedIcon size={24} className="text-[#FFD166]" />
            </div>
            <div>
              <p className="text-sm font-semibold text-white">{form.title || "Achievement Title"}</p>
              <p className="text-xs text-gray-400 mt-0.5">{form.description || "Achievement description..."}</p>
              <span className="inline-block mt-1.5 text-[10px] font-medium px-2 py-0.5 rounded-full
                ...
                {CRITERIA_OPTIONS.find(c => c.value === form.criteriaType)?.label}: {form.criteriaValue}
              </span>
            </div>
          </div>
